#!/usr/bin/env python3
"""Create Databricks Knowledge Assistants from project-scoped or stash ka_*.yml files.
...
import os as _os
_PROJECT_DIR = _os.environ.get("PROJECT_DIR", "").strip()
if _PROJECT_DIR:
    CONFIG_DIR = Path(_PROJECT_DIR) / "conf"
else:
    CONFIG_DIR = Path("/dev/null")  # no project active -- nothing to scan
...
Examples:
  uv run python scripts/py/ka/create_kas_from_yml.py
  uv run python scripts/py/ka/create_kas_from_yml.py ka_config.yml --no-wait
  uv run python scripts/py/ka/create_kas_from_yml.py --dry-run
  uv run python scripts/py/ka/create_kas_from_yml.py --skip-existing
        """,
    )
    parser.add_argument("configs", nargs="*", metavar="CONFIG", help="Specific YAML file(s). If omitted, all ka_*.yml in project conf dir.")
